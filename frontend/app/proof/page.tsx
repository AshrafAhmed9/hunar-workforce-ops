...
export default function Proof() {
  ...
  return <main><p className="eyebrow">Evidence</p><h1>Local, durable call evidence.</h1><p className="lede">This page reads the application database, not a live provider feed, so historical outcomes remain available if provider access is revoked.</p><section className="panel"><h2>Call records</h2>{state && <p className="lede" aria-live="polite">{state}</p>}{calls.length > 0 && <><p className="lede">In-progress calls refresh every 3 seconds.</p><table><thead><tr><th>Request</th><th>Outcome</th><th>Extracted result</th><th>Recording</th></tr></thead><tbody>{calls.map(call => <tr key={call.id}><td>{call.request_id}</td><td className="status">{call.status}</td><td className="wrap"><code>{JSON.stringify(call.result)}</code></td><td>{call.recording_url ? <a href={call.recording_url}>Open recording</a> : "Pending"}</td></tr>)}</tbody></table></>}</section><p className="notice">No third-party call data or recordings are displayed, stored, or fetched by this app.</p></main>;
}
